...
No OpenAI API key is needed for the prepared judge demo. The custom upload flow requires the visitor to add their own key before the build action is enabled. It is used per request, never written to the database or logs, and held in memory only while the job runs. Keys never belong in the repository.
...
### Downloadable custom-upload example

Judges can also download the fictional multidisciplinary test set and attach all of its files through the combined uploader: [open the ProyectTest folder](https://github.com/CesarReGu/NotZero/tree/main/examplesfinalad/ProyectTest) or [download it as a ZIP](https://github.com/CesarReGu/NotZero/raw/main/examplesfinalad/ProyectTest.zip). The example contains only fictional academic evidence and readable source files. The prepared demo is the no-key path; the custom live path requires the visitor's own OpenAI API key.
...
The typed server-side Responses API adapters use ChatGPT for each stage of the live flow. GPT-5.6 provides the substantive comparison and guided-program reasoning required by the submission, while deterministic application code bounds and validates every model request. The visitor-facing live flow remains a sequence of schema-constrained calls:
...
For a field no curated pack covers, a preceding pack stage generates the current-practice reference the two stages above compare against, and it is what makes NotZero domain-agnostic in form. ChatGPT locates job postings with the Responses API web-search tool, then normalizes those postings into canonical requirements and role clusters without searching again. The server assembles the full pack deterministically from the located postings, computing reciprocal source/requirement mappings and mention counts rather than trusting them. A generated pack is labeled illustrative rather than reviewed, and regulated fields such as law, nursing, and accounting still need their own reviewed sources and safety rules before their conclusions can be relied on. Curated packs, when they match, are always preferred over generation.

All stages enforce output-token ceilings, prompt-cache keys, strict JSON Schema output, and a bounded upstream request timeout before independent Zod validation. Development diagnostics are disabled in the submission build. When enabled locally, they record only redacted lifecycle and model-response metadata and omit prompts, uploaded text, response content, API keys, and cookies.
...
The custom upload flow requires a visitor key before the build action is enabled. The key is sent with each request, never written to the database or logs, held in the visitor's browser tab and, while a job is running, in a process-memory map that is swept within the hour and dropped when the job finishes or on reset. A deployment can also keep a server-side key for controlled operation, subject to spending limits and the circuit breaker.
...
At the beginning, ChatGPT helped locate reliable research on effective study methods and turn the product idea into a practical roadmap. It then helped implement the core pillars: the evidence ledger, trust rules, current-practice comparison, Knowledge Bridge report, and judge-ready experience. During live testing it also helped resolve blocking failures, including an evidence excerpt that could not be resolved to `app.py`, an unsupported relationship attached to scikit-learn, and stalled oversized requests. The fixes added stricter provenance handling, bounded batching, checkpointed retries, and clearer validation without requiring users to prepare perfectly organized files.
