...
export function KnowledgeBridgeReportView({ report, ledger, subjectLabel, pack: packProp }: { report: KnowledgeBridgeReport; ledger: EvidenceLedger; subjectLabel?: string; pack?: CurrentPracticePack }) {
  ...
  const generatedBannerText = webSearched
    ? `NotZero has no human-reviewed market pack for ${pack.field} yet, so GPT-5.6 located current job postings for your field, target, and location by web search on ${pack.observedThrough}, and NotZero derived the requirement counts from them. The postings are real and linked, but no human reviewed them. Treat the conclusions as illustrative and check specifics before relying on them.`
  ...
  const openingsLimitText = archetypes
    ...
    : webSearched
    ? `These are real postings GPT-5.6 located on ${pack.observedThrough}; each link opens where it was found so the sample stays checkable. Some may have closed since.`
  ...
  const trustMarketText = archetypes
    ...
    : webSearched
    ? `GPT-5.6 located these postings by web search on ${pack.observedThrough}. Counts come from the located postings, not an open-ended web summary, but no human reviewed them.`
    ...
    <section className="bridge-report" aria-labelledby="bridge-report-title" ref={reportRef}>
      ...
      <section className="openings" aria-labelledby="openings-title">
        <div className="section-intro">
          ...
          <p>Requirement counts come from these {pack.sources.length} {sourcesWord}. Here they are individually, closest first, so the sample is something you can check rather than a number to trust. Reading a {archetypes ? "role and its live search" : "posting"} you already reach is the fastest way to see which words to use.</p>
        </div>
