...
export function assembleGroundedPack(scan: JobPostingScan, synthesis: GroundedSynthesisModelOutput, fieldContext: FieldContext): CurrentPracticePack {
  ...
  const sources: PackSource[] = scan.postings
    .map((posting) => ({
      ...
      sourceType: "employer_job_posting" as const,
      usageBasis: `Current ${fieldContext.field} opening located by GPT-5.6 web search on ${scan.retrievedAt}. Requirement labels are normalized from the posting text and were not human-reviewed.`,
  ...
  return finishPack({
    ...
    methodology: [
      `NotZero has no human-reviewed market pack for ${fieldContext.field}, so GPT-5.6 located current job postings by web search on ${scan.retrievedAt} and NotZero derived the requirement counts from them.`,
      "Requirement counts record how many of the located postings state each requirement. The postings are real and linked, but no human reviewed them.",
    ...
    limitations: [
      ...
      "Each posting links to where it was found. Postings change or close, and the requirement labels are the model's normalization of the posting text. Verify specifics before relying on them.",
...
export function finalizeGeneratedReport(report: KnowledgeBridgeReport, pack: CurrentPracticePack): KnowledgeBridgeReport {
  ...
  const provenance = webSearched
    ? `The current-practice reference for ${pack.field} was built from real job postings GPT-5.6 located by web search on ${pack.observedThrough}. The postings are real and linked, but no human reviewed them, so this is an illustrative synthesis rather than a curated dataset.`
  ...
  const countsNote = webSearched
    ? "Market counts describe how many of the located postings state each requirement, not a human-reviewed sample. Verify specifics and links before relying on them."
